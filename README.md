...
문서 상태는 브라우저 `localStorage`에 자동 저장됩니다. 새로고침하면 마지막
작업 문서가 복원됩니다. `Export document`로 전체 문서 상태를 JSON으로
내보낼 수 있고, `Import document`로 다시 불러올 수 있습니다. `Reset document`
는 확인 후 로컬 draft를 삭제하고 기본 문서로 되돌립니다.

사용자 계정, 여러 저장 문서, 클라우드 저장소는 아직 포함하지 않습니다.
...
- 파일 크기: 이미지당 최대 2 MB
- 개수: 최대 20개
- 같은 파일명을 다시 업로드하면 기존 자산을 교체합니다.
- 업로드한 이미지는 자산 목록에서 삭제할 수 있습니다.
